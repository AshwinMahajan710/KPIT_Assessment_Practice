...
# ans = reduce(lambda acc,x: acc*x , nums, 1)
# print(ans)
...
# ans = list(map(lambda x: sum(1 for ch in x if ch in "AEIOUaeiou"), words))
# print(ans)
...
seen = set()
# ans = list (x for x in lst if not x in seen or not seen.add(x)))
# print(ans)
...
# from functools import reduce
# nums = [1,2,3,4,5]
# # print(reduce(lambda x,y: x+y,list(map(lambda x: x*x,list(filter(lambda x: x%2!=0,nums))))))
# # < - - - - - approach 2 - - - - - - - >
# print(sum(ele*ele for ele in nums if ele%2!=0))
...
nums = [2,4,6]
from functools import reduce
ans = list(itertools.accumulate(nums))
# ans = reduce(lambda acc,y: acc + [y] if not acc else acc + [acc[-1] + y],nums,[])
print(ans)
...
# data = ['to', 'level', 'test', 'radar', 3.14]
# print(list(filter(lambda x: isinstance(x,str) and x[0] == x[-1],data)))